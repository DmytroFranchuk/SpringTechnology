...
public class ProductController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<ProductDto> updateProductByIdPatch(
            @PathVariable("id") Long id, @RequestBody ProductDto productDto) {
    ...
    public ResponseEntity<List<ProductDto>> getAllProductsByStatus(@PathVariable("status") StatusType status) {
        List<ProductDto> products = productService.getAllProductsWhereStatusTypeIs(status);
        return ResponseEntity.ok(products);
    }

    @GetMapping("/type/{type}/currency/{code}")
    public ResponseEntity<List<ProductDto>> getAllProductsByProductTypeAndCurrencyCode(
